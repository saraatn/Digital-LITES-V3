...
(function() {
  ...
  var viewLeft Element = document.querySelector('#viewLeft');
...
function updateSimulationProgress() {
  ...
  if (progressText) progressText.innerText = percentage + '% (' + completedCount + '/7 Stations)';
  
  if (completedCount === totalRequiredStations && progressText) {
    progressText.innerText = "100% Completed! Simulator Mastered.";
  }
}
    ...
    mutation.addedNodes.forEach(function(node) {
      ...
      if (videoElement) {
        videoElement.addEventListener('ended', function() {
          if (window.currentScene && stationMap[window.currentScene.data.id]) {
            const currentStationName = stationMap[window.currentScene.data.id];
...
// --- GLOBAL DYNAMIC VIDEO HOTSPOT CLOSE HANDLER ---
